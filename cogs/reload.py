...
class Reload(commands.Cog):
    ...
    async def reload(self, interaction: Interaction, extension: str = SlashOption(
            name="extension",
        ...
        if not interaction.user.guild_permissions.administrator:
            logger.error(f"Extension [{extension}] wasn't reloaded because [{interaction.user.name}] didn't have sufficient permission")
        ...
        try:
            ...
            return await interaction.response.send_message(f"Extension `{extension}` has been reloaded successfully.", ephemeral=True)
